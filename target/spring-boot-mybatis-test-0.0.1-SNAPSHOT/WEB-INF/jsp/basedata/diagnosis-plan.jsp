...
<body>
	...
	<div class='result' style="height: 250px;">
		<p><span>诊断方案：</span><input type="text" placeholder='输入方案名称' id="treatmentPlan"  maxlength="64"></p>
	...
	<div class="wrap">
		...
		<div class="maincon">
			...
			<!-- 右侧 -->
			<div class="mainRight">
				<p class='top'><span>基础信息管理</span><em>></em><span class="qiDetail">诊疗方案管理</span></p>
				<div class='mainList'>
					<div class='divs'>
						<p class='conLeft'  onclick="location.reload()">全部信息</p>
						<ul class="conRight">

							<li><em class="require addyp">新增诊疗方案</em></li>
						</ul>
					</div>
					<div class="tab">
						<table>
	...
	<script type="text/javascript">
        ...
        function init(pageNo) {
            ...
            $.ajax({
                url:"${basePath}/diagnosis-plan.json",
                type:"post",
                data:params,
                success:function(json){
                    if(json.result.code==0){
                        ...
                        for(var i=0;i<json.data.totalList.length;i++){
                            ...
                            html += '<tr>';
                            html += '<td>'+item.treatmentPlan+'</td>';
                            html += '<td><em class="modify addyp" onclick="addyp(\''+item.id+'\',\''+item.treatmentPlan+'\')">修改</em>/<em class="modify remove2" onclick="remove2(\''+item.id+'\')">删除</em></td>';
                            html += '</tr>';
                        }
        ...
        function save(){
            var params={};
            params.id = i_id;
            params.treatmentPlan = $.trim($("#treatmentPlan").val());
            ...
            i_id = "0";
            $.ajax({
                url:"${basePath}/diagnosisadd-plan.json",
                type:"post",
                data:params,
                success:function(json){
                    if(json.result.code==0){
        ...
        function del(){
            ...
            params.id = i_id;
            $.ajax({
                url:"${basePath}/diagnosisdel-plan.json",
                type:"post",
                data:params,
                success:function(json){
                    if(json.result.code==0){
