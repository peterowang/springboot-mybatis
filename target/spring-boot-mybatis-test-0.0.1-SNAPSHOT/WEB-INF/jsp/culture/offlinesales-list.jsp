...
<body>
	...
	<div class="wrap">
		...
		<div class="maincon">
			...
			<!-- 右侧 -->
			<div class="mainRight">
				<p class='top'><span>养殖管理<em>></em></span><span class="qiDetail">线下销售</span></p>
				<div class='mainList'>
					<div class='divs'>
    ...
    function init(pageNo,flag) {
		...
		params.pageSize = 10;
        $.ajax({
            url:"${basePath}/offlinesales-list.json",
            type:"post",
            data:params,
            success:function(json){
                if(json.result.code==0){
                    ...
                    for(var i=0;i<json.data.totalList.length;i++){
                        ...
                        html += '<tr><td><a href="${basePath}/livestock-'+item.livestockId+'.html">'+item.equNumber+'</a></td><td>'+item.salesTime+'</td><td>'+item.customerName+'</td><td>'+item.contactWay+'</td><td>'+item.unitPrice+'元</td><td><a href="${basePath}/enterprise-'+item.enterpriseId+'.html">'+item.enterpriseName+'</a></td><td><a href="${basePath}/person-'+item.personnelId+'.html">'+item.personnelName+'</a></td></tr>';
                    }
                    html = '<table><thead><tr><th>耳标编号</th><th>时间</th><th>销售客户</th><th>联系方式</th><th>单价</th><th>企业</th><th>操作人员</th></tr></thead>' + html + '</tbody></table>';
