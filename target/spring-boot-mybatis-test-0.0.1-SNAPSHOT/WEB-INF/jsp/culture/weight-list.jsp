...
<body>
	...
	<div class="wrap">
		...
		<div class="maincon">
			...
			<!-- 右侧 -->
			<div class="mainRight">
				<p class='top'><span>养殖管理<em>></em></span><span class="qiDetail">称重记录</span></p>
				<div class='mainList'>
					<div class='divs'>
						<p class='conLeft' onclick="flush();">全部履历</p>
						<ul class="conRight">
							<shiro:hasRole name="admin">
							<li><span>企业：</span><select name="enterpriseId" id="select1">
    ...
    function init(pageNo,flag) {
	   ...
	   params.number = $("#input3").get(0).value;
        $.ajax({
            url:"${basePath}/weight-list.json",
            type:"post",
            data:params,
            success:function(json){
                if(json.result.code==0){
                    ...
                    for(var i=0;i<json.data.totalList.length;i++){
                        ...
                        html += '<tr><td><a href="${basePath}/livestock-'+item.livestockId+'.html">'+item.number+'</a></td><td>'+item.typeName+'</td><td>'+item.weighTime+'</td><td>'+item.weight+'公斤</td><td><a href="${basePath}/enterprise-'+item.enterpriseId+'.html">'+item.enterpriseName+'</a></td><td><a href="${basePath}/person-'+item.weighId+'.html">'+item.weighName+'</a></td></tr>';
                    }
                    html = '<table><thead><tr><th>耳标编号</th><th>品种</th><th>称重时间</th><th>重量</th><th>企业</th><th>称重人员</th></tr></thead>' + html + '</tbody></table>';
