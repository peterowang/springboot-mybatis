...
<body>
	...
	<div class="wrap">
		...
		<div class="maincon">
			...
			<!-- 右侧 -->
			<div class="mainRight">
				<p class='top'><span>养殖管理<em>></em></span><span class="qiDetail">诊疗记录</span></p>
				<div class='mainList'>
					<div class='divs'>
    ...
    function init(pageNo) {
        ...
        $.ajax({
            url:"${basePath}/diagnosis-list.json",
            type:"post",
            data:params,
            success:function(json){
                if(json.result.code==0){
                    ...
                    for(var i=0;i<json.data.totalList.length;i++){
                        ...
                        html += '<tr><td>'+item.documentNumber+'</td><td><a href="${basePath}/livestock-'+item.livestockId+'.html">'+item.equNumber+'</a></td><td>'+item.time+'</td><td>'+item.dragName+'</td><td>'+item.result+'</td><td>'+item.treatmentPlanName+'</td><td><a href="${basePath}/enterprise-'+item.enterpriseId+'.html">'+item.enterpriseName+'</a></td><td><a href="${basePath}/person-'+item.personnelId+'.html">'+item.personnelName+'</a></td></tr>';
                    }
                    html = '<table><thead><tr><th>单据编号</th><th>耳标编号</th><th>时间</th><th>用药名称</th><th>诊疗结果</th><th>诊疗方案</th><th>企业</th><th>诊疗人员</th></tr></thead>' + html + '</tbody></table>';
