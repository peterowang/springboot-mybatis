...
<body>
	...
	<div class="wrap">
		...
		<div class="maincon">
			...
			<!-- 右侧 -->
			<div class="mainRight">
				<p class='top'><span>养殖管理<em>></em></span><span class="qiDetail">喂养记录</span></p>
				<div class='mainList'>
					<div class='divs'>
    ...
    function init(pageNo,flag) {
        ...
        $.ajax({
            url:"${basePath}/feeding-list.json",
            type:"post",
            data:params,
            success:function(json){
                if(json.result.code==0){
                    ...
                    for(var i=0;i<json.data.totalList.length;i++){
                        ...
                        html += '<tr><td><a href="${basePath}/livestock-'+item.livestockId+'.html">'+item.equNumber+'</a></td><td>'+item.documentNumber+'</td><td>'+item.feedingTime+'</td><td>'+item.feedingName+'</td><td>'+item.grass+'</td><td><a href="${basePath}/enterprise-'+item.enterpriseId+'.html">'+item.enterpriseName+'</a></td><td><a href="${basePath}/person-'+item.personnelId+'.html">'+item.personnelName+'</a></td></tr>';
                    }
                    html = '<table><thead><tr><th>耳标编号</th><th>单据编号</th><th>时间</th><th>饲料</th><th>牧草</th><th>企业</th><th>喂养人员</th></tr></thead>' + html + '</tbody></table>';
