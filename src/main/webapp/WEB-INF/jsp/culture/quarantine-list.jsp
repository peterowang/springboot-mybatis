 ...
 <%@include file="../include/left.jsp"%>
    ...
    function init(pageNo,flag) {
	   ...
	   params.endTime = $("#input3").get(0).value;
        $.ajax({
            url:"${basePath}/quarantine-list.json",
            type:"post",
            data:params,
            success:function(json){
                if(json.result.code==0){
                    ...
                    for(var i=0;i<json.data.totalList.length;i++){
                        ...
                        html += '<tr><td><a href="${basePath}/quarantine-'+item.documentNumber+'.html">'+item.documentNumber+'</a></td><td>'+item.createTime+'</td><td><a href="${basePath}/enterprise-'+item.enterpriseId+'.html">'+item.enterpriseName+'</a></td><td><a href="${basePath}/person-'+item.quarantinePersonnelId+'.html">'+item.personName+'</a></td></tr>';
                    }
